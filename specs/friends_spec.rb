...
class TestFriends < MiniTest::Test
  ...
  def test_favourte_foods
    favourite_foods = ["charcuterie","soup","bread","ratatouille", "stew","spaghetti","spinach"]
    assert_equal(favourite_foods, favourite_foods(@people))
  end
  ...
  def test_no_friends
    zero_friends = ["Dave"]
    assert_equal(zero_friends, no_friends(@people))
  end

  def test_total_age
    result = total_age(@people)
    assert_equal(83, result)
  end

  def test_many_friends
    result = ["Rick", "Keith"]
    assert_equal(result, many_friends(@people))
  end
  # Optional, VERY TOUGH
